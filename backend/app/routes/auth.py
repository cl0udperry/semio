...
async def generate_api_key(
    email: str = Form(...),
    password: str = Form(...),
    key_name: str = Form(..., description="Name for the API key (e.g., 'gitlab-pipeline')"),
    expires_in_days: int = Form(30, description="Days until key expires")
):
    """
    ...
    """
    try:
        # Authenticate user and store API key in a single session
...
async def list_api_keys(
    ...
    """
    try:
        # Authenticate user and get API keys in a single session
        from app.database import SessionLocal
        db = SessionLocal()
        try:
            user = AuthService.authenticate_user(db, email, password)
...
async def revoke_api_key(
    email: str = Form(...),
    password: str = Form(...),
    key_name: str = Form(..., description="Name of the API key to revoke")
):
    """
    Revoke an API key.
    Requires user authentication.
    """
    try:
        # Authenticate user and revoke API key in a single session
        from app.database import SessionLocal
        db = SessionLocal()
        try:
            user = AuthService.authenticate_user(db, email, password)
